...
- Cloud upload sweeper: cloud sessions (Colab) run on a small ephemeral disk,
  so a background thread deletes each job's uploaded/intermediate files
  90 minutes after their last activity (AVC_UPLOAD_TTL_MINUTES overrides).
  Jobs still processing are never touched, and finished exports live in
  exports/, not temp/, so results survive. Local runs are unaffected.
"""
...
def start_upload_sweeper() -> None:
    ...
    logger.info(
        "Cloud session: uploads auto-delete after %.0f minutes of inactivity.",
        UPLOAD_TTL_MINUTES,
    )
