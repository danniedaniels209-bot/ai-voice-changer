...
import os
import librosa
# Vendored patch: text_to_sequence is only needed by BaseSpeakerTTS (which
# this app doesn't use) and drags in heavy text-processing deps — import it
# lazily inside get_text instead of at module level.
...
class OpenVoiceBaseClass(object):
    ...
    def load_ckpt(self, ckpt_path):
        # Vendored patch: weights_only=True — never unpickle arbitrary objects
...
class ToneColorConverter(OpenVoiceBaseClass):
    def __init__(self, *args, **kwargs):
        # Vendored patch: enable_watermark is consumed here, not by the base
        # class — pop it so super().__init__ doesn't reject it.
